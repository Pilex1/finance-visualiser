...
class Database:
    def __init__(self):
        ...
        self.table_transaction = sql.Table(
            ...
            sql.Column("description_id", sql.VARCHAR, sql.ForeignKey("description.id")),
            sql.Column("description", sql.VARCHAR),
            sql.Column("description_original", sql.VARCHAR),
            sql.Column("balance", sql.DECIMAL(10, 2)),
            sql.UniqueConstraint("date", "amount", "balance", "description_original"),
        )
        self.table_description = sql.Table(
            ...
            sql.Column("category_id", sql.VARCHAR, sql.ForeignKey("category.id"))
        )
    ...
    def initialise_empty_tables(self):
        ...
        self.conn.execute(psql.insert(self.table_category).values(categories_df.to_dict(orient="records")))
        self.conn.commit()

    @staticmethod
    def get_suburb_info(desc: str) -> tuple[str, Optional[str]]:
        """
        ...
        def extract_suburb(y: str, i: int):
            ...
            if match:
                ...
                match_remainder = re.search("^" + Constants.regex_post_suburb, remainder)
                if match_remainder:
        ...
        if result[1] is None:
            ...
            if match:
                start_index = match.span()[0]
                match_truncated = re.search(f"({Constants.regex_suburb_truncated})$", desc.lower()[:start_index].strip())
                if match_truncated:
    ...
    def process_records(filename: str) -> pd.DataFrame:
        df = pd.read_csv(filename,
                        names=["date", "amount", "description_original", "balance"],
                        parse_dates=["date"],
                        date_format="%d/%m/%Y")
        ...
        result = df["description"].apply(Database.get_suburb_info).apply(pd.Series)
    ...
    def process_description(desc: str) -> tuple[str, bool]:
        ...
        if desc in DescRules.display_name_dict: return DescRules.display_name_dict[desc], True
        
        for s in DescRules.starts_with_set:
            if desc.lower().startswith(s.lower()): return s, True

        for k, v in DescRules.starts_with_dict.items():
            if desc.lower().startswith(k.lower()): return v, True

        for k, v in DescRules.regex_dict.items():
            if re.search(k, desc, flags=re.IGNORECASE): return v, True
    ...
    def add_records(self, csv_file: str):
        ...
        descriptions = pd.DataFrame.from_records(descriptions, columns=["id", "processed"])
        descriptions["category_id"] = descriptions["id"].apply(lambda x: Constants.category_dict[x] if x in Constants.category_dict else None)
        self.conn.execute(psql.insert(self.table_description).values(descriptions.to_dict(orient="records")).on_conflict_do_nothing())
        ...
        self.conn.execute(psql.insert(self.table_transaction).values(df.to_dict(orient="records")).on_conflict_do_nothing())
    ...
    def get_transactions_groupby(self):
        ...
        coalesce_date = func.coalesce(transaction.c["value_date"], transaction.c["date"]).label("date")
        query = (
            sql.select(
                coalesce_date,
            ...
            .select_from(
                transaction.join(description, transaction.c["description_id"] == description.c["id"], isouter=True)
            )
